function [uav,target,targetResources,nextBroadcast] = uavlogic(uav,target,dt,l,b,targetLocations,targetResources,broadcast,nextBroadcast)
% main logic is implemented in this function

global trustGain targetMeasurements targetValue targetThreshold trustFlag actualPerformance targetResoources
global st

Rmin = 50; % minimum radius of turn
...
% update uav position
uav.Position = [x y] + [cos(psi) sin(psi)]*V*dt;
...
if uav.Status == 'InCoalition'
    ...
    if (x-xt)^2+(y-yt)^2<2*10^2
        ...
        if remainingResources==0
            if trustFlag == 0
                target(uav.Target).Resources = updateTargetResources([targetResoources(uav.Target,1) 0 0], actualPerformance(uav.coalitionMembers),target(uav.Target).visit);
           ...
           % disp(['Target ' num2str(uav.Target) ' destroyed!'])
%            if targetMeasurements(uav.Target) > targetThreshold
%                targetValue = targetValue + targetMeasurements(uav.Target);
%            end
        end
        ...
        % update rule 
        if trustFlag == 1
            uav.coalitionMembers
            for icm = 1:length(uav.coalitionMembers)
                uav.trustValues(uav.coalitionMembers(icm)) = uav.trustValues(uav.coalitionMembers(icm)) + trustGain*(uav.coalitionPerformance-uav.trustValues(uav.coalitionMembers(icm)));
            end
        end
        uav.Target = [];
        uav.TargetLocation = [];
