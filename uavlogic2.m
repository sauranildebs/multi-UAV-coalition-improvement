function [uav,target,targetResources,nextBroadcast] = uavlogic2(uav,target,dt,l,b,targetLocations,targetResources,broadcast,nextBroadcast)
% main logic is implemented in this function

global trustGain targetMeasurements targetValue targetThreshold trustFlag actualPerformance updatedTargetResources
global st
...
if uav.Status == 'InCoalition'
    ...
    if (x-xt)^2+(y-yt)^2<2*10^2
        ...
        if remainingResources==0
            target(uav.Target).Resources = updateTargetResources_v1([updatedTargetResources(uav.Target,1) 0 0], actualPerformance(uav.coalitionMembers));
