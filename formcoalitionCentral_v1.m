function [nextBroadcast, targetArray] = formcoalitionCentral_v1(uav,target,broadcast,nextBroadcast,ctime, targetArray)
% function to form coalition. contains the coalition formation logic

global trustFlag actualPerformance Phat numCoalitions numResources
...
if ~isempty(coalitionCandidateResources)
    ...
    if (sum(coalitionCandidateResources,1)-broadcast(iBroadcast).ResourceRequired)>=0
        ...
        while sum(coalitionResources,1)>=broadcast(iBroadcast).ResourceRequired
            ...
            if totalRemResources >= broadcast(iBroadcast).ResourceRequired
...
targetArray(target).allocatedTime = ctime;
        ...
        numResources = numResources + ceil(targetResourceRequired);
    end
