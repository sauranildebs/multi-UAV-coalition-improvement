    function [uav,target,targetResources,nextBroadcast, endflag] = uavlogicCentral(uav,target,dt,l,b,targetLocations,targetResources,broadcast,nextBroadcast, endflag, ctime)
...
if ~isempty(uav.CoalitionLeader)
    [uav, nextBroadcast, target] = doifcoalitioncoordinatorCentral(uav,broadcast,nextBroadcast,ctime, target);
end
...
% update uav position
uav.tempPosition = [x y] + [cos(psi) sin(psi)]*V*dt;
...
%         end
        ...
        bdaMember = uav.coalitionMembers(1);
        if uav.Id == bdaMember
            uav.Status = 'BDAwaiting*';
            cost = dubinsdistance(uav, uav.TargetLocation, Rmin);
            uav.TurnRadius = findturnradius(uav,uav.TargetLocation,cost, Rmin);
            uav.TurnDirection = -sign(sum(cross([cos(psi) sin(psi) 0],[([uav.TargetLocation 0]-[x y 0])])));
        else
            uav.Status = 'InSearch***';
        end
